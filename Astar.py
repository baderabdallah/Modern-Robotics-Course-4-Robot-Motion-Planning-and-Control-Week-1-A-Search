...
if __name__ == "__main__":
    ...
    nodesFileName = nodesFileName.decode(
        "utf-8")+u'\\Scene5_example\\nodes.csv'
    ...
    edgesFileName = edgesFileName.decode(
        "utf-8")+u'\\Scene5_example\\edges.csv'
        ...
        for eachChild in current.openNode.childNode:
            ...
            if not any(x.openNode == eachChild for x in closedNodeList):
                if any(x.openNode.order == eachChild.order for x in openNodeList):
                    popOut = next(
                        (x for x in openNodeList if x.openNode.order == eachChild.order), None)
                    openNodeList.pop(openNodeList.index(popOut))
                    openNodeList.append(OpenListItem(
                        eachChild, estTotalCost[eachChild.order-1]))
                else:
                    openNodeList.append(OpenListItem(
                        eachChild, estTotalCost[eachChild.order-1]))
